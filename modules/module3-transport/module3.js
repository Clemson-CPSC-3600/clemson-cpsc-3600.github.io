...
// Get Module 3 data
const module3 = MODULES.find(m => m.number === 3);

if (!module3) {
  console.error('Module 3 configuration not found');
}

// Create progress tracker instance
let progressTracker = null;
if (module3) {
  progressTracker = new ProgressTracker(3, module3);
}

// Initialize breadcrumb navigation
function initBreadcrumb() {
  const breadcrumbContainer = document.getElementById('breadcrumb-container');
  if (breadcrumbContainer && module3) {
    const breadcrumb = Breadcrumb.createCustom([
      { label: 'Home', href: '../../index.html' },
      { label: `Module ${module3.number}: ${module3.title}`, href: null }
    ]);
...
function initLessons() {
  ...
  if (!lessonsContainer || !module3 || !progressTracker) return;

  module3.lessons.forEach(lesson => {
...
function initDemos() {
  ...
  if (!demosContainer || !module3 || !progressTracker) return;

  module3.demos.forEach(demo => {
...
function initPractice() {
  ...
  if (!practiceContainer || !module3 || !progressTracker) return;

  module3.practice.forEach(practice => {
...
function initProgressTracking() {
  ...
  console.log(`Module 3 Progress: ${completed}/${total} (${percentage}%)`);
}
...
function init() {
  ...
  document.body.classList.add('module-3');
  document.documentElement.style.setProperty('--color-module', module3.color);
}
