...
// Get Module 5 data
const module5 = MODULES.find(m => m.number === 5);

if (!module5) {
  console.error('Module 5 configuration not found');
}

// Create progress tracker instance
let progressTracker = null;
if (module5) {
  progressTracker = new ProgressTracker(5, module5);
}

// Initialize breadcrumb navigation
function initBreadcrumb() {
  const breadcrumbContainer = document.getElementById('breadcrumb-container');
  if (breadcrumbContainer && module5) {
    const breadcrumb = Breadcrumb.createCustom([
      { label: 'Home', href: '../../index.html' },
      { label: `Module ${module5.number}: ${module5.title}`, href: null }
    ]);
...
function initLessons() {
  ...
  if (!lessonsContainer || !module5 || !progressTracker) return;
...
function initDemos() {
  ...
  if (!demosContainer || !module5 || !progressTracker) return;

  module5.demos.forEach(demo => {
...
function initPractice() {
  ...
  if (!practiceContainer || !module5 || !progressTracker) return;

  module5.practice.forEach(practice => {
...
function initProgressTracking() {
  ...
  console.log(`Module 5 Progress: ${completed}/${total} (${percentage}%)`);
}
...
function init() {
  ...
  document.body.classList.add('module-5');
  document.documentElement.style.setProperty('--color-module', module5.color);
}
